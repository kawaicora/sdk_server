...
class Chat {
    constructor() {
        ...
        this.RegisterMicrophone();
        $("#send_pic_button").click(this.SendMediaMessage.bind(this));
        $("#send_msg_button").click(this.SendChatMessage.bind(this));
        ...
        SocketIOMaster.on('user-joined-room', async (data) => {
            this.logger.debug(`新用户加入房间 ${JSON.stringify(data, null, 2)}`);
            CommonUtils.MsgBox(`${data.username}加入了房间 ${data.room_info.title}`);
            
        });

        SocketIOMaster.on('joined-room', async (data) => {
            sessionStorage.setItem("room",data.room_id);
            this.logger.debug(`成功加入房间 ${JSON.stringify(data.room_info, null, 2)}`);
            CommonUtils.MsgBox(`${data.username}加入了房间 ${data.room_info.title}`);
            this.LoadHistoryMessages(data.room_id)
            this.logger.debug(`加载历史消息`);
    ...
    JoinRoom(room_id) {
        try {
            ...
            this.logger.error('加入房间失败:', e);
            CommonUtils.MsgBox('加入房间失败: ' + e.message);
        }
    }
    LoadHistoryMessages(room) {
        try {
            SocketIOMaster.emit('get-room-message-history', { room: room });

        } catch (e) {
            this.logger.error('加载历史消息失败:', e);
            CommonUtils.MsgBox('加载历史消息失败: ' + e.message);
        }
    }
    InitMsgTemplate(msgId) {
      // 从 message_list 中读取模板
      this.MsgTemplateCache = $(".message_list #msg_id").clone();
      // 清空message_list的模板只留缓存
      $(".message_list #msg_id").remove();
      this.MsgTemplateCache.removeClass("hidden");
      console.log("模板读取成功：", this.MsgTemplateCache.length ? "是" : "否");
    ...
    AppendMessage(msg) {
        ...
        if (this.MsgTemplateCache == null)
            this.InitMsgTemplate()
        var obj = this.MsgTemplateCache.clone() //复制一个避免全局变化
        
        if ( msg.sender_uid== sessionStorage.getItem("uid")) {
    ...
    SendChatMessage() {
        ...
        if (message) {
            SocketIOMaster.emit('chat-message', { message: message, type: "html" ,room_id:sessionStorage.getItem("room")});
            input.value = '';
        }
    }
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (response.retcode === 0) {
                            ...
                            SocketIOMaster.emit('chat-message', { type: response.data.type, message: response.data.message });
                        } else {
